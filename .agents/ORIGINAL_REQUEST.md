...
## 2026-09-07T06:43:12Z

Rewrite the football simulation backend from Python to Go (Golang) in structured chunks. Complete and verify Chunk 1 (Core Domain Models, Value Math, Growth Engine, and dataset.json Ingestion), then stop and report results for user inspection before proceeding to subsequent chunks.

Working directory: c:\Users\Izyan\Downloads\General\football_sim
Integrity mode: development

## Requirements

### R1. Chunk 1 Core Domain Models, Valuations & Growth Engine
Implement high-performance Go domain models (`Player`, `Club`, `Standings`), realistic financial/valuation curves (`BaselineValue`, `ClampValue`, `FormatCurrency`), personality archetypes, and biometric growth systems (`GrowthEngine`, `BiometricProfile`, `TechnicalAttributes`) with age decline and youth development.

### R2. Data Ingestion & Canonical Wonderkid Setup
Ingest `dataset.json` (96 clubs, 2,294 players) into native Go memory without data loss. Maintain strict squad deduplication (0 duplicate players) and initialize the 12 canonical U-14 wonderkids at age 14 in middle school with exact potentials (93–96). Implement academy regen youth intake.

### R3. Chunk Completion Stop Policy
Once Chunk 1 is fully implemented and all tests pass, stop and report the status and code structure to the user for inspection. Do not proceed to Chunk 2 until the user gives the green light.
