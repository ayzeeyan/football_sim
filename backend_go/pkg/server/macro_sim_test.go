...
type clubTableState struct {
	ClubID                                                   string
	Played, Won, Drawn, Lost, GoalsFor, GoalsAgainst, Points int
}
...
func TestMacroMonthStopsAtDeadlineWeekBeforeTransition(t *testing.T) {
	srv, ts := setupTestServer(t)
	defer srv.Stop()
	defer ts.Close()
	...
	for i := 0; i < 9; i++ { // process Weeks 1–9; Week 10 is now active
		srv.TransferEngine.AdvanceOpenWindow()
	}
	first, status := srv.runMacroSimulationLocked("month")
	if status != http.StatusOK || first.NewSeasonStarted || srv.TransferEngine.CurrentWeek != 12 || !srv.TransferEngine.IsWindowOpen() {
		srv.worldMu.Unlock()
		t.Fatalf("month should stop with deadline Week 12 open: status=%d result=%+v week=%d open=%v", status, first, srv.TransferEngine.CurrentWeek, srv.TransferEngine.IsWindowOpen())
	}
	if first.WeeksAdvanced != 2 {
		srv.worldMu.Unlock()
		t.Fatalf("deadline cap advanced %d weeks, want 2", first.WeeksAdvanced)
	}
	second, status := srv.runMacroSimulationLocked("month")
	srv.worldMu.Unlock()
	if status != http.StatusOK || !second.NewSeasonStarted || second.WeeksAdvanced != 1 {
		t.Fatalf("deadline month should process Week 12 and transition: status=%d result=%+v", status, second)
	}
}
...
func TestFirstTransferWindowBudgetUsesCompletedSeasonReputation(t *testing.T) {
	...
	if status != http.StatusOK || out.Status != "success" {
		t.Fatalf("first offseason week failed: status=%d out=%+v", status, out)
	}
	if tm.ReputationAppliedSeason != tm.SeasonName {
		t.Fatalf("reputation marker=%q want %q", tm.ReputationAppliedSeason, tm.SeasonName)
	}
	if tm.ClubsList[0].Identity.Reputation == beforeRep {
		t.Fatal("completed season did not change champion reputation")
	}
	...
	if tm.ClubsList[0].Finances.Balance > 0 && expected > tm.ClubsList[0].Finances.Balance {
		expected = tm.ClubsList[0].Finances.Balance
	}
	if got := tm.ClubsList[0].Finances.TransferBudget; got != expected {
		t.Fatalf("week 1 budget=%d want %d from updated reputation=%d", got, expected, identity.Reputation)
	}
}
...
func TestMacroTransferWindowWaitsForIncompleteCup(t *testing.T) {
	...
	if status != http.StatusConflict {
		srv.worldMu.Unlock()
		t.Fatalf("incomplete cup should block window start with HTTP 409, got %d", status)
	}
...
func TestSeasonResetRouteRequiresTwelveTransferWeeks(t *testing.T) {
	...
	if resp.StatusCode != http.StatusBadRequest {
		t.Fatalf("expected guarded reset status 400, got %d", resp.StatusCode)
	}
	srv.worldMu.RLock()
