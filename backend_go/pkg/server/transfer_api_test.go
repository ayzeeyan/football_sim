...
func TestTransfersBoardMatchesReactContractWhileWindowClosed(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("advance while closed: %v", err)
	}
	var closed map[string]interface{}
	decodeJSONBody(t, resp, &closed)
	if resp.StatusCode != http.StatusBadRequest {
		t.Fatalf("closed advance status=%d", resp.StatusCode)
	}
	if closed["detail"] != "The window opens when the season ends." {
		t.Fatalf("closed advance detail=%v", closed["detail"])
	}
}
...
func TestTransferBidAndAdvanceDuringCareerWindow(t *testing.T) {
	...
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("bid status=%d payload=%v", resp.StatusCode, bid)
	}
	if bid["formatted_bid"] == nil || bid["stage_name"] != "INQUIRY" {
		t.Fatalf("bid payload missing React fields: %v", bid)
	}
	...
	if err != nil {
		t.Fatalf("repeat bid: %v", err)
	}
	var bid2 map[string]interface{}
	decodeJSONBody(t, resp2, &bid2)
	if bid2["negotiation_id"] != bid["negotiation_id"] {
		t.Fatalf("repeat bid opened a second talk: %v vs %v", bid["negotiation_id"], bid2["negotiation_id"])
	}

	saved, err := persistence.LoadCareer(srv.savePath)
	if err != nil {
		t.Fatalf("bid did not persist: %v", err)
	}
	if saved.Transfers.CurrentDay != dayBefore {
		t.Fatalf("bid should not advance the window day, got %d from %d", saved.Transfers.CurrentDay, dayBefore)
	}
	...
	if respAdv.StatusCode != http.StatusOK {
		t.Fatalf("advance status=%d payload=%v", respAdv.StatusCode, board)
	}
	if board["is_window_open"] != true {
		t.Fatalf("advance should keep the career window open: %v", board["is_window_open"])
	}
	if board["window_day"].(float64) != float64(dayBefore+1) {
		t.Fatalf("window_day=%v want %d", board["window_day"], dayBefore+1)
	}
	negs, _ := board["active_negotiations"].([]interface{})
	if len(negs) == 0 {
		t.Fatal("expected negotiations after a bid and a market day")
	}
	first, _ := negs[0].(map[string]interface{})
	if first["formatted_bid"] == nil || first["player"] == nil {
		t.Fatalf("negotiation not serialized for React: %v", first)
	}
	warchests, _ := board["warchests"].([]interface{})
	if len(warchests) != 12 {
		t.Fatalf("warchests=%d, want 12", len(warchests))
	}
	saved, err = persistence.LoadCareer(srv.savePath)
	if err != nil {
		t.Fatalf("advance persist: %v", err)
	}
	if saved.Transfers.CurrentDay != dayBefore+1 {
		t.Fatalf("saved window day=%d want %d", saved.Transfers.CurrentDay, dayBefore+1)
	}
}

func TestTransferAdvanceWalksWeek1To2AndPersists(t *testing.T) {
	srv, ts := setupTestServer(t)
	defer ts.Close()
	defer srv.Stop()
	...
	srv.TournamentManager.SeasonPhase = "transfer_window"
	srv.TransferEngine.BeginOffSeasonWindow()
	if srv.TransferEngine.CurrentWeek != 1 {
		srv.worldMu.Unlock()
		t.Fatalf("fresh window week=%d want 1", srv.TransferEngine.CurrentWeek)
	}
	srv.worldMu.Unlock()
	...
	decodeJSONBody(t, resp, &board)
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("advance status=%d payload=%v", resp.StatusCode, board)
	}
	if board["window_week"].(float64) != 2 {
		t.Fatalf("window_week=%v want 2", board["window_week"])
	}
	...
	if saved.Transfers.CurrentWeek != 2 {
		t.Fatalf("saved week=%d want 2", saved.Transfers.CurrentWeek)
	}
}
