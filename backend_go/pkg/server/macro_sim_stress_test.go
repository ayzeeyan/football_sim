...
func TestTenSeasonMacroSoakValidatesEveryBoundary(t *testing.T) {
	...
	for season := 1; season <= seasons; season++ {
		...
		if seasonResult.WeeksAdvanced != tournament.LeagueRounds {
			t.Fatalf("season %d advanced %d league weeks, want %d", season, seasonResult.WeeksAdvanced, tournament.LeagueRounds)
		}
		...
		if len(categories) < 5 {
			srv.worldMu.RUnlock()
			t.Fatalf("season %d produced incomplete award ceremony", season)
		}
		for _, cat := range categories {
			...
			if len(noms) == 0 {
				...
				if winnerID != "" || cat["winner"] != nil {
					srv.worldMu.RUnlock()
					t.Fatalf("season %d award %v has winner without eligible nominees", season, cat["key"])
				}
				continue
			}
			if winnerID == "" {
				srv.worldMu.RUnlock()
				t.Fatalf("season %d award %v has nominees but no explicit winner", season, cat["key"])
			}
			found := false
			for _, n := range noms {
				if n["player_id"] == winnerID {
					found = true
					break
				}
			}
			if !found {
				srv.worldMu.RUnlock()
				t.Fatalf("season %d award %v winner is not a finalist", season, cat["key"])
			}
		}
		for _, club := range srv.TournamentManager.ClubsList {
			if club.Played != tournament.LeagueRounds {
				srv.worldMu.RUnlock()
				t.Fatalf("season %d club %s played %d league games want %d", season, club.ClubID, club.Played, tournament.LeagueRounds)
			}
			if club.Finances.TransferBudget < 0 || club.Finances.Balance < 0 || club.Finances.TransferBudget > club.Finances.Balance {
				srv.worldMu.RUnlock()
				t.Fatalf("season %d club %s has invalid finances", season, club.ClubID)
			}
			if club.Identity.Reputation < minRep {
				minRep = club.Identity.Reputation
			}
			if club.Identity.Reputation > maxRep {
				maxRep = club.Identity.Reputation
			}
			for _, p := range club.Squad {
				if transfers.IsCanonicalWonderkid(p) {
					if !transfers.IsDesignatedSuperLeagueClub(p.ClubID) {
						srv.worldMu.RUnlock()
						t.Fatalf("season %d canonical wonderkid %s escaped ecosystem to %s", season, p.PlayerID, p.ClubID)
					}
					if p.OVR < minWK {
						minWK = p.OVR
					}
