...
func (tm *TournamentManager) runWeeklyTicks(completedMW int) {
	...
	for _, club := range tm.ClubsList {
		...
		if prodigy == nil || tm.GrowthEngine == nil {
			continue
		}
		...
		if prodigy.MentorName != "" && prodigy.MentorOVR > 0 {
			...
			growthEvents = append(growthEvents, tm.GrowthEngine.ApplyMentorshipTick(
				prodigy.PlayerID, prodigy.FullName, growth.MentorshipOptions{
					MentorName: &mName, MentorOVR: &mOVR, Personality: &pers,
				},
			)...)
		}
		if tm.RNG != nil && tm.RNG.Float64() <= 0.22 {
			...
			if err == nil && res["status"] != "error" {
				growthEvents = append(growthEvents, fmt.Sprintf("%s staff ran %s training for %s.", club.ShortName, focus, prodigy.FullName))
			}
		}
		prodigy.OVR = tm.GrowthEngine.CalculateOVR(prodigy.PlayerID, prodigy.Category)
		if attrs, ok := tm.GrowthEngine.Attributes[prodigy.PlayerID]; ok && attrs != nil {
	...
	if tm.TransferEngine != nil {
		tm.TransferEngine.RevertToBaselines()
	}
	...
	if completedMW == 24 {
		tm.PushInbox("nxgn",
			"NXGN 2027: The 50 Best Wonderkids in World Football Ranked",
			"Goal's annual NXGN rankings are officially out! The 12 Franchise Prodigies headline the global elite.",
			completedMW, nil, "", "")
	}
...
func (tm *TournamentManager) pickPlayerOfTheWeek(mw int) {
	...
	consider := func(f *Fixture) {
		if f.Status != "finished" || f.Report == nil || f.Report.MOTM == nil {
			return
		}
		motm := f.Report.MOTM
		rating := 0.0
		...
		if side == "away" {
			club = tm.Clubs[f.AwayID]
		}
		clubName, clubShort := "", ""
		if club != nil {
			clubName, clubShort = club.ClubName, club.ShortName
		}
		c := cand{
			rating: rating,
			goals:  motm.MatchGoals,
			ovr:    motm.OVR,
			row: map[string]interface{}{
				"player_id":    motm.PlayerID,
				"full_name":    motm.FullName,
				"position":     motm.Position,
				"ovr":          motm.OVR,
				"rating":       rating,
				"goals":        motm.MatchGoals,
				"assists":      motm.MatchAssists,
				"club_name":    clubName,
				"club_short":   clubShort,
				"is_wonderkid": motm.IsWK,
				"matchweek":    mw,
			},
		}
		if best == nil || c.rating > best.rating || (c.rating == best.rating && c.goals > best.goals) || (c.rating == best.rating && c.goals == best.goals && c.ovr > best.ovr) {
			cp := c
			best = &cp
		}
	}
	for i := range tm.Fixtures {
		if tm.Fixtures[i].Matchweek == mw {
			consider(&tm.Fixtures[i])
		}
	}
	for i := range tm.UCLFixtures {
		if tm.UCLFixtures[i].Matchweek == mw {
			consider(&tm.UCLFixtures[i])
		}
	}
	for i := range tm.SuperCupFixtures {
		if tm.SuperCupFixtures[i].Matchweek == mw {
			consider(&tm.SuperCupFixtures[i])
		}
	}
	if best != nil {
		tm.PlayerOfTheWeek = best.row
		tm.PushInbox("honour",
			fmt.Sprintf("Player of the week: %s", best.row["full_name"]),
			fmt.Sprintf("%s posted a %.1f against the week's best night.", best.row["full_name"], best.rating),
			mw, nil, fmt.Sprintf("%v", best.row["player_id"]), "")
	}
}
...
func (tm *TournamentManager) maybeCrownMonth(endMW int) {
	...
	for _, b := range MonthBands {
		if b.Hi == endMW || (endMW == tm.MaxMatchweeks && endMW >= b.Lo && endMW <= b.Hi) {
			bandLo, bandHi, name = b.Lo, b.Hi, b.Name
			found = true
			break
		}
	}
	...
	for _, a := range tm.MonthlyAwards {
		if fmt.Sprintf("%v", a["month"]) == name {
			return
		}
	}
	type slot struct {
	...
	addPack := func(clubID string, rows []matchreport.MatchPlayerRow) {
		...
		for _, row := range rows {
			if !row.Played || row.Minutes <= 0 || row.Rating == nil {
				continue
			}
			s := tally[row.PlayerID]
			if s == nil {
				s = &slot{
					playerID: row.PlayerID, fullName: row.FullName, position: row.Position, ovr: row.OVR,
					isWonderkid: row.IsWK,
				}
				if club != nil {
					s.clubName, s.clubShort = club.ClubName, club.ShortName
				}
				tally[row.PlayerID] = s
			}
			s.ratings = append(s.ratings, *row.Rating)
			s.goals += row.MatchGoals
		}
	}
	...
	for mw := bandLo; mw <= bandHi && mw <= endMW; mw++ {
		for i := range tm.Fixtures {
			if tm.Fixtures[i].Matchweek == mw {
				addRows(&tm.Fixtures[i])
			}
		}
		for i := range tm.UCLFixtures {
			if tm.UCLFixtures[i].Matchweek == mw {
				addRows(&tm.UCLFixtures[i])
			}
		}
		for i := range tm.SuperCupFixtures {
			if tm.SuperCupFixtures[i].Matchweek == mw {
				addRows(&tm.SuperCupFixtures[i])
			}
		}
	}
	if len(tally) == 0 {
	...
	for _, s := range tally {
		...
		avg /= float64(len(s.ratings))
		if best == nil || avg > bestAvg || (avg == bestAvg && len(s.ratings) > len(best.ratings)) {
			best = s
			bestAvg = avg
		}
	}
	...
	award := map[string]interface{}{
		...
		"ovr":          best.ovr,
		"rating":       avg,
		"apps":         len(best.ratings),
		"goals":        best.goals,
		"club_name":    best.clubName,
		"club_short":   best.clubShort,
		"is_wonderkid": best.isWonderkid,
	}
	...
	tm.PushInbox("honour",
		fmt.Sprintf("%s player of the month: %s", name, best.fullName),
		fmt.Sprintf("%.2f average across %d appearances.", avg, len(best.ratings)),
		endMW, nil, best.playerID, "")
}
...
func (tm *TournamentManager) decayDerbyHeat(mw int) {
	...
	for k, v := range tm.DerbyHeat {
		if v > 50 && !tm.DerbiesPlayedThisMW[k] {
			tm.DerbyHeat[k] = v - 1
		}
	}
	_ = mw
...
func (tm *TournamentManager) maybeExamWeekInbox(completedMW int) {
	next := completedMW + 1
	if !models.IsExamWeek(next) {
		return
	}
	sitting := 0
	for _, c := range tm.ClubsList {
		for _, p := range c.Squad {
			if p.UniverseWonderkid && p.SchoolConflict("super-league", next) {
				sitting++
			}
		}
	}
	...
	tm.PushInbox("youth",
		"Exam week: enrolled prodigies sit",
		"School comes first while they are enrolled. Cup nights are already off-limits in middle school.",
		completedMW, nil, "", "")
}
