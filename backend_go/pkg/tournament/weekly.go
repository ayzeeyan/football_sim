...
func (tm *TournamentManager) runWeeklyTicks(completedMW int) {
	...
	for _, club := range tm.ClubsList {
		...
		// Every prodigy at the club trains, in squad order: transfers can
		// leave two franchise kids sharing a dressing room, and the second
		// must not starve. Single-prodigy clubs behave exactly as before.
	...
	if tm.TransferEngine != nil {
		...
		// January is driven by the shared season calendar, not by an
		// offseason-only UI action. Each completed league week processes one
		// finite winter-market week; the engine closes at Week 4 without ever
		// exposing a synthetic Week 5.
...
func (tm *TournamentManager) pickPlayerOfTheWeek(mw int) {
	...
	consider := func(f *Fixture) {
		...
		c := cand{rating: rating, goals: motm.MatchGoals, ovr: motm.OVR, playerID: motm.PlayerID, row: map[string]interface{}{
		...
		if best == nil || c.rating > best.rating || (c.rating == best.rating && c.goals > best.goals) ||
			(c.rating == best.rating && c.goals == best.goals && c.ovr > best.ovr) ||
			(c.rating == best.rating && c.goals == best.goals && c.ovr == best.ovr && c.playerID < best.playerID) {
			cp := c
			best = &cp
		}
	}
...
func (tm *TournamentManager) maybeCrownMonth(endMW int) {
	...
	for _, b := range MonthBands {
		if b.Hi == endMW || (endMW == tm.MaxMatchweeks && endMW >= b.Lo && endMW <= b.Hi) {
			bandLo, bandHi, name, found = b.Lo, b.Hi, b.Name, true
			break
		}
	}
	if !found {
	...
	type slot struct {
		playerID, fullName, position string
		ovr                          int
		ratings                      []float64
		goals                        int
		clubName, clubShort          string
		isWonderkid                  bool
	}
	...
	for _, s := range tally {
		...
		if best == nil || avg > bestAvg || (avg == bestAvg && len(s.ratings) > len(best.ratings)) || (avg == bestAvg && len(s.ratings) == len(best.ratings) && s.playerID < best.playerID) {
			best, bestAvg = s, avg
		}
	}
	if best == nil {
