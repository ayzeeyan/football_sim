  ...
  return (
    <div
      className={cx(
        ...
        isWinner
          ? 'border-brass bg-brass/[0.1] shadow-raised'
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 mt-5">
        {category.nominees.map((n, i) => (
          <NomineeCard
            ...
            stats={n.stats_line}
            ovr={n.ovr}
            isWinner={revealed && category.winner?.full_name === n.full_name}
            dimmed={revealed}
            index={i}
          />
        ))}
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="text-center">
        <span className="px-3 py-1 rounded-full bg-brass/15 border border-brass/40 text-brass text-[11px] font-mono font-bold uppercase tracking-wider">
          Global Football Honors
        </span>
        ...
        <p className="text-[13px] text-sage mt-1">Continental jury votes for the supreme player of the season.</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-4 items-end pt-4 pb-2">
        ...
        {top1 && (
          <div className="flex flex-col items-center">
            ...
            <div className="mb-2 text-center">
              <span className="inline-block px-2.5 py-0.5 rounded-full bg-brass/20 text-brass border border-brass/50 text-[10px] font-mono font-bold uppercase tracking-wider mb-1">
                Winner
              </span>
              ...
              <p className="text-[12px] text-brass font-mono font-semibold mt-0.5">
                {top1.goals}G · {top1.assists}A · {top1.ovr} OVR
              </p>
            </div>
              ...
              <div key={item.player_id} className="py-2 flex items-center justify-between text-[13px]">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    {item.is_wonderkid && (
                      <span className="ml-2 px-1.5 py-0.5 rounded text-[10px] font-mono bg-brass/15 text-brass font-bold">
                        U-14
                      </span>
                    )}
                  </div>
      ...
      <div className="space-y-4">
        {lines.map((line) => (
          <div key={line.label} className="space-y-2">
            ...
            <div className={cx(
              'grid gap-2.5',
              line.players.length === 1 ? 'grid-cols-1 max-w-sm mx-auto' :
              line.players.length === 4 ? 'grid-cols-2 sm:grid-cols-4' : 'grid-cols-1 sm:grid-cols-3'
            )}>
              {line.players.map((card) => {
                ...
                return (
                  <div
                    key={card.player_id}
                    className="p-3.5 rounded-xl border border-brass/35 bg-brass/[0.05] hover:border-brass/70 transition-all flex flex-col justify-between"
                  >
                    <div className="flex items-center justify-between gap-2">
                      <span className="px-1.5 py-0.5 rounded bg-cardLight text-sage font-mono text-[10px] font-bold">
                    ...
                    <div className="flex items-center gap-2.5 my-2">
                      {url ? (
                        <img src={url} alt="" className="w-8 h-8 rounded-md bg-bone p-0.5 object-contain shrink-0" />
                      ) : (
                        <span className="w-8 h-8 rounded-md bg-cardLight shrink-0" />
                      )}
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="text-center">
        ...
        <p className="text-[13px] text-sage mt-1">Honouring the mastermind whose tactical philosophy conquered Europe.</p>
      </div>
      ...
      <div className="p-6 rounded-2xl border-2 border-brass/60 bg-gradient-to-br from-cardLight via-ink to-cardLight shadow-2xl space-y-5">
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 text-center sm:text-left">
          {url ? (
            <img src={url} alt="" className="w-16 h-16 rounded-xl bg-bone p-1 object-contain border border-brass/50 shadow-md shrink-0" />
          ) : (
            <div className="w-16 h-16 rounded-xl bg-cardLight border border-brass/50 shrink-0" />
          )}
          <div className="flex-1">
            <span className="px-2 py-0.5 rounded bg-brass text-ink font-mono font-bold text-[11px] uppercase tracking-wider">
              {manager.club_name}
            </span>
            <h4 className="font-display font-bold text-[24px] text-bone mt-1.5">
              {manager.name || manager.manager_name}
            </h4>
            <p className="text-[13px] text-brass font-mono mt-0.5">
              {manager.tactic} · {manager.style}
            </p>
          </div>
        </div>

        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
          <div className="p-3 rounded-xl bg-ink/60 border border-line">
            <p className="text-[10px] text-sage uppercase font-mono">League Finish</p>
            <p className="font-mono font-bold text-[18px] text-brass mt-1">#{manager.actual_finish}</p>
          </div>
          <div className="p-3 rounded-xl bg-ink/60 border border-line">
            <p className="text-[10px] text-sage uppercase font-mono">Expectation</p>
            <p className="font-mono font-bold text-[18px] text-bone mt-1">#{manager.expected_finish}</p>
          </div>
          <div className="p-3 rounded-xl bg-pitchtone/10 border border-pitchtone/40">
            <p className="text-[10px] text-pitchtone uppercase font-mono">Overperformed</p>
            <p className="font-mono font-bold text-[18px] text-pitchtone mt-1">+{manager.outperformed_places} places</p>
          </div>
          <div className="p-3 rounded-xl bg-ink/60 border border-line">
            <p className="text-[10px] text-sage uppercase font-mono">Trophies Lifted</p>
            <p className="font-mono font-bold text-[18px] text-bone mt-1">{manager.trophies_won}</p>
          </div>
        </div>

        {manager.accolade && (
          <blockquote className="p-4 rounded-xl bg-ink/70 border-l-4 border-brass text-[14px] text-bone/90 italic leading-relaxed">
            "{manager.accolade}"
          </blockquote>
        )}
      </div>
      ...
      <div className="text-center pt-2">
        <button
          onClick={onClose}
          className="px-8 py-3 rounded-xl bg-cardLight hover:bg-cardHover text-bone border border-line text-[14px] font-bold transition-colors inline-flex items-center gap-2"
        >
      ...
      <ModalHeader
        title={
          <span className="flex items-center gap-2">
            <Trophy size={19} className="text-brass" />
            <span>Awards night{data ? `, ${data.season_name}` : ''}</span>
          </span>
        }
      ...
      <div className="p-6">
        ...
        {!loading && data && stage === 'intro' && (
          <div className="text-center py-8 animate-fade-in max-w-lg mx-auto">
            ...
            <p className="text-[13px] text-sage mt-2 leading-relaxed">
              Experience the crowning moments of European football: 4 major awards, the Ballon d'Or podium, Team of the Season, and Manager of the Year.
            </p>
            <button
              onClick={() => {
                soundManager.playWhistle();
              ...
              className="mt-6 px-10 py-3.5 rounded-xl bg-brass hover:bg-[#D4AF4D] text-ink text-[15px] font-bold transition-colors inline-flex items-center gap-2 shadow-lg"
            >
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-center gap-1.5 p-1 bg-ink/60 border border-line rounded-xl overflow-x-auto">
              <button
                ...
                className={cx(
                  'px-3 py-1.5 rounded-lg text-[12px] font-semibold transition-colors shrink-0',
                  stage === 'categories' ? 'bg-brass text-ink' : 'text-sage hover:text-bone',
                )}
              >
                Award Envelopes ({catIndex + 1}/{categories.length || 4})
              </button>
              <button
                type="button"
                onClick={() => setStage('ballondor')}
                className={cx(
                  'px-3 py-1.5 rounded-lg text-[12px] font-semibold transition-colors shrink-0',
                  stage === 'ballondor' ? 'bg-brass text-ink' : 'text-sage hover:text-bone',
                )}
              >
                Ballon d'Or 🥇
              </button>
              <button
                type="button"
                onClick={() => setStage('tots')}
                className={cx(
                  'px-3 py-1.5 rounded-lg text-[12px] font-semibold transition-colors shrink-0',
                  stage === 'tots' ? 'bg-brass text-ink' : 'text-sage hover:text-bone',
                )}
              >
                Team of the Season
              </button>
              <button
                ...
                className={cx(
                  'px-3 py-1.5 rounded-lg text-[12px] font-semibold transition-colors shrink-0',
                  stage === 'manager' ? 'bg-brass text-ink' : 'text-sage hover:text-bone',
                )}
              >
                Manager of the Year
              </button>
            ...
            {stage === 'ballondor' && (
              <BallonDorPodiumView
                rankings={ballonDorList}
                onNext={() => setStage('tots')}
              />
            )}
            ...
            {stage === 'tots' && tots && (
              <TeamOfTheSeasonView
                tots={tots}
                onNext={() => setStage('manager')}
              />
            )}

            {stage === 'manager' && moty && (
              <ManagerOfTheYearView
                manager={moty}
                onClose={onClose}
              />
            )}
